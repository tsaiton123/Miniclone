...
struct FileSystemView: View {
    ...
    @StateObject private var semanticSearch = SemanticSearchService()
        ...
        NavigationStack(path: $viewModel.navigationPath) {
            DashboardLayout(searchText: $viewModel.searchText, selectedTab: $selectedTab, onSettings: {
                ...
                VStack {
                    ...
                    ScrollView {
                        ...
                        // Semantic search results (appear when query ≥ 3 chars)
                        if !viewModel.searchText.isEmpty &&
                           (semanticSearch.isSearching || !semanticSearch.results.isEmpty) {
                            SemanticMatchesSection(
                                results: semanticSearch.results,
                                allItems: allItems,
                                isSearching: semanticSearch.isSearching
                            ) { note in
                                viewModel.navigationPath.append(note)
                            }
                            .padding(.bottom, 12)
                        }
                    }
            ...
            .onChange(of: viewModel.searchText) { newQuery in
                if newQuery.isEmpty {
                    ...
                    semanticSearch.search(query: newQuery)
                }
            }
